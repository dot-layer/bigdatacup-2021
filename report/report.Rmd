...
In the second part, we raise the bar higher by performing a time series analysis in which we evaluate how the probability of scoring a goal evolve in time following a faceoff.
Finally, the last section revisits the previous ones with a much larger dataset in order to corroborate our initial findings.

## The overall impact of winning a faceoff {#overall}

The first thing to validate is that winning a faceoff gives an advantage as expected.
A simple way of doing this would be to fit a logistic regression where each sequence would be an observation, and use a goal indicator as the response variable.
By including the faceoff outcome as a covariate, we will be able to confirm if there is any statistical evidence that winning a faceoff have a positive effect.
Even though increasing the probability of scoring a goal or reducing the probability of allowing one both give a significant edge, we will focus on the former in this section.
...
With this in mind, one can have a look at a summary of the data we will be working with in Table \@ref(tab:features1)—OZ stands for Offensive Zone.
An excerpt of the training dataset we will use for the logistic regression, including the features we created is displayed in \@ref(tab:preview).
...
To get a crude idea of the impact in goal scoring of winning/loosing a faceoff, our first logistic regression only included an intercept and `FO_win` as a covariate.
We obtained $\beta_{\rm FO\_win} =$ `r round(summary$coefficients[2,][1], 3)`, which suggests that winning an offensive faceoff does increase the chances to score a goal during the sequence that ensues.
However, as it will be the case for the remaining of this section, the associated p-value is too high to make any conclusion.
This is probably due to the low number of observations we have.

One may suggest that we add some more covariates to better explain which factors may favorise goal scoring—we already gave some intuitive ones earlier.
When adding the binary `FO_OZ` covariate in the model, we obtain $\beta_{\rm FO\_win} =$ `r round(summary2$coefficients[2,][1], 3)` and $\beta_{\rm FO\_OZ} =$ `r round(summary2$coefficients[3,][1], 3)`, both covariate having a positive impact.
Since there is probably an important interaction between the faceoff zone and outcome, we fitted a last logistic model including the `FO_win` * `FO_OZ` interaction.
The coefficient of interest are now $\beta_{\rm FO\_win} =$ `r round(summary3$coefficients[2,][1], 3)`, $\beta_{\rm FO\_OZ} =$ `r round(summary3$coefficients[3,][1], 3)` and $\beta_{\rm FO\_win + FO\_OZ} =$ `r round(summary3$coefficients[4,][1], 3)` suggesting that winning a faceoff or having an offensive zone faceoff gives great opportunities, but that the real deal is to win offensive zone faceoffs.

However, with the limited data we have, it was difficult to gain significant signal from other covariates.
One could look at \@ref(tab:features2) for a one-way analysis and to see how strength of play would be difficult to include in a model with so little observations.
