...
The noise in the barcharts of Figure \ref{fig:barcharts} caused by the lack of data points makes a bit it hard to interpret them.
To overcome such difficulty, we now construct a smooth version of these latter using a generalized additive model (*gam*).
We relegate the more formal description of the underlying model to the **Technical details** section at the end of the report.
...
In fact, it must be said that the results are not "significant", in the statistical sense.
...
\rule{\textwidth}{1pt}
## Technical details
#### Figure \ref{fig:gam} and \ref{fig:gam-nhl}
...
Two models, one using $t^* = log(t+1)$ and another using $t^* = t$ are shown in Figure \ref{fig:gam-nhl}.
...
Looking back at the fact that the various probabilities estimated were no greater than 0.5\% = 0.005, it would have been surprising to obtain statistically significant results with the \textcolor{red}{MM} offensive faceoffs of the Erie dataset.
...
Table \@ref(tab:nhl1)) provides a basic summary of the NHL dataset.
...
It seems worthwhile pointing out that we now work with \textcolor{red}{XX} faceoffs that lead to \textcolor{red}{YY} goals.
...
The result is displayed on the right panel of \ref{fig:barchart-nhl}.
