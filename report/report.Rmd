...
The main objective of this study is to quantify how much. In other words, in leveraging the data at our disposal, we will try to answer the following question:
...
A non-exhaustive list of important factors may include the current score, the time remaining on the clock, the zone in which the faceoff is taken, or the strength of play (e.g. power play).
Many will also argue that—even though there is not a single coach who overlooks the importance of winning faceoffs—some game plans are simply better designed to make the most of an offensive zone faceoff win while others are more effective at salvaging a faceoff loss in the defensive zone.
...
In the first part, we analyze the faceoff effect on a sequence basis, where a sequence is defined as the time between two successive faceoffs.
In the second part, we raise the bar higher by performing a time series analysis in which we evaluate how the probability of scoring a goal evolve over time after the faceoff.
Finally, in the last section, we perform the aforementioned analyses on a much larger dataset so as to corroborate our initial findings.
...
A simple way of doing this is to treat each sequence an observation and fit a logistic regression with a goal indicator as the response variable.
By including the faceoff outcome as a covariate, we will be able to validate that there is statistical evidence that winning a faceoff have a positive effect.
Although reducing the probability of allowing a goal is as important as increasing the probability of scoring a goal, we will focus on the latter in this section.
...
In order to work with relatively homogeneous data, we made the decision to only use on the [scouting dataset](https://raw.githubusercontent.com/bigdatacup/Big-Data-Cup-2021/main/hackathon_scouting.csv).
Indeed, mixing different leagues may introduce noise as each of them may imply a different style of hockey.
Furthermore, the fact that every games in the data involve the Erie Otters will allow us to create models from a single perspective.
...
With that in mind, let us take a look at the data we will be working with.
A summary of it is provided in Table \@ref(tab:features1)—OZ stands for Offensive Zone—and an excerpt of the training dataset we will use for the logistic regression, including the features we created, is displayed in Table \@ref(tab:preview).
...
To get a crude idea of the impact in goal scoring of winning/loosing a faceoff, we first fitted a logistic regression with only an intercept and `FO_win` as a covariate.
We obtained $\beta_{\rm FO\_win} =$ `r round(summary$coefficients[2,][1], 3)`, which suggests that winning an offensive faceoff does increase the chances of scoring a goal during the sequence that ensues.
However, the associated p-value was too high to make any statistically backed conclusion.
...
We conclude this section with one-way analysis, shown in Table \@ref(tab:features2), that groups the faceoff by "situation" (penalty kill, even strengths, powerplay).
Although analyzing the impact of winning faceoffs in such specific situations would have been very interesting, Table \@ref(tab:features2) suggests that this could impossible with so little observations.
...
In order to convince ourselves that our intuition is nevertheless backed by data, we now perform a similar analysis using the 2018-19 and 2019-20 NHL data.\footnote{We fetched the data using \href{https://jplecavalier.github.io/tidynhl/}{\texttt{tidynhl}}, an \texttt{R} package developed on top of the NHL Stats and Records APIs.}
